...
/* JavaScript content from js/aes/AES_Encryption-impl.js in folder common */
/*
 * Encrypt data using aes encryption
 */
function EncryptData(data,key){
	GibberishAES.size(192);      // 128 , 192 or 256 bits encryption mode
	
	var reversedData=reverseIt(data);
	
	ct = GibberishAES.enc(reversedData,key);
	return ct;
}

/*
 * Decrypt cyphertext using aes decryption
 */

function DecrytData(ct, key){
	GibberishAES.size(192);
	hexData=GibberishAES.dec(ct, key);
	
	var plText=reverseInv(hexData);
	
	return plText;
}

/**
 *  the costum functions :
 */


function reverseIt(input){
	var b= convertToHex(input);
	var d1=hexToBinary(b).result;
	var d2=inverser(d1);
	var t=binaryToHex(d2).result;
	return t;
}

function reverseInv(hex){  
	var t1=hexToBinary(hex).result;
	var d3=inverser(t1);
	var d=binaryToHex(d3).result;
	var dd=convertFromHex(d);
	return dd;
}
